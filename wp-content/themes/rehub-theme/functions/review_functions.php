...
function rehub_get_user_rate($schema='admin', $type = 'post', $customid = ''){
	wp_enqueue_script( 'rh-userrating', get_template_directory_uri() . '/js/userrating.js', array( 'jquery', 'rehub' ), 1.0, true );
	if ($type == 'post') {
		global $post;
		$postid = $post->ID;
	}
...
function rehub_get_review(){
	...
	<div class="rate_bar_wrap<?php if ((rehub_option('type_user_review') == 'full_review') && ($postAverage !='0' && $postAverage !='' )) {echo ' two_rev';} ?><?php if (rehub_option('color_type_review') == 'multicolor') {echo ' colored_rate_bar';} ?>">		
		<?php if ($overal_score !='0') :?>
			<?php $reviewheading = vp_metabox('rehub_post.review_post.0.review_post_heading'); ?>
			<?php if ($reviewheading) :?>
...
function getCommentLike_re( $comment_text  ) {
	...
	$output .= '<span class="us-rev-vote-up'.$already.'" data-post_id="'.$post_id.'" data-informer="'.$comm_plus_count.'"><i class="rhicon rhi-thumbs-up"></i> <span class="comm_help_title">'.__('Helpful', 'rehub-theme').'</span>(<span class="help_up_count" id="commhelpplus'.$post_id.'">'.$comm_plus_count.'</span>)</span>'; 
	$output .= '<span class="us-rev-vote-down'.$already.'" data-post_id="'.$post_id.'" data-informer="'.$comm_minus_count.'"><i class="rhicon rhi-thumbs-down"></i> <span class="comm_help_title">'.__('Unhelpful', 'rehub-theme').'</span>(<span class="help_up_count" id="commhelpminus'.$post_id.'">'.$comm_minus_count.'</span>)</span>'; 
