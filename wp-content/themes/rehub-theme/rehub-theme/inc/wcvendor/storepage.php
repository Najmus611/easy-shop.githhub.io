	    ...
	    <aside class="rh-mini-sidebar user-profile-div floatleft tabletblockdisplay">
	    	<div class="rh-cartbox widget">
	            <div>
                    ...
                    <div><i class="rhicon rhi-user"></i> <?php esc_html_e( 'Registration', 'rehub-theme' );  echo ': ' .date_i18n( get_option( "date_format" ), strtotime( $vendor->user_registered )); ?></div>
	                    ...
	                    <?php if (!empty($mycredpoint)) :?><div><i class="rhicon rhi-chart-bar"></i><?php echo esc_html($mycredlabel); ?>: <?php echo esc_attr($mycredpoint); ?> </div><?php endif; ?>
	                </div>
					<?php if ( class_exists( 'WCVendors_Pro' ) ) : ?>
