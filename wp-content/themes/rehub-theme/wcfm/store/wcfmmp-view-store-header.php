...
<div id="wcfm_store_header" class="rh-container header_area rh-shadow3 rh-flex-columns rehub-sec-smooth tabletsblockdisplay">
	<div id="wcvendor_profile_logo" class="wcvendor_profile_cell text-center user-profile-div">
		...
		<?php if( $titleposition == 'on_header' ) { ?>
			<div class="profile-stats mb15">
				...
				?> 
				<div class="text-center"><i class="rhicon rhi-heartbeat"></i><?php esc_html_e( 'Product Votes', 'rehub-theme' ); echo ': ' . $count_p_votes; ?></div>
				<div class="text-center"><i class="rhicon rhi-briefcase"></i><?php esc_html_e( 'Total submitted', 'rehub-theme' ); echo ': ' . $totaldeals; ?></div>
	...
	<div id="wcvendor_profile_act_desc" class="wcvendor_profile_cell rh-flex-grow1">
		<div class="address">
			...
			<?php if( $address && ( $store_info['store_hide_address'] == 'no' ) && $WCFM->wcfm_vendor_support->wcfm_vendor_has_capability( $vendor_id, 'vendor_address' ) ) { ?>
				<p><i class="wcfm rhicon rhi-map-marker-alt" aria-hidden="true"></i><span><?php echo esc_html($address); ?></span></p>
			<?php } ?>
			...
			<div>
				<?php if( $phone && ( $store_info['store_hide_phone'] == 'no' ) && $WCFM->wcfm_vendor_support->wcfm_vendor_has_capability( $vendor_id, 'vendor_phone' ) ) { ?>
					<div class="store_info_parallal" style="margin-right: 10px;"><i class="wcfm rhicon rhi-phone" aria-hidden="true"></i><span><?php echo esc_html($phone); ?></span></div>
				<?php } ?>
				
				<?php do_action( 'wcfmmp_store_after_phone', $vendor_id ); ?>
				<?php do_action( 'wcfmmp_store_before_email', $vendor_id ); ?>
				
				<?php if( $email && ( $store_info['store_hide_email'] == 'no' ) && $WCFM->wcfm_vendor_support->wcfm_vendor_has_capability( $vendor_id, 'vendor_email' ) ) { ?>
					<div class="store_info_parallal"><i class="wcfm rhicon rhi-envelope" aria-hidden="true"></i><span><a href="#"><?php echo esc_html($email); ?></a></span></div>
				<?php } ?>
				<div class="spacer"></div>  
			</div>
