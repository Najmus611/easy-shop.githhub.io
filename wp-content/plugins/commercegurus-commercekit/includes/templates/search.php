		...
		<main id="main" class="site-main">
			...
			<div class="ajs-tabs-content-wrap">
				...
				<div class="ajs-tabs-content active" id="ajs-products">
					...
					$attributes['page']  = absint( empty( $_GET['product-page'] ) ? 1 : $_GET['product-page'] ); // WPCS: input var ok, CSRF ok.
					$result_total        = 0;
					...
					if ( $products && $products->ids ) {
						...
						if ( wc_get_loop_prop( 'total' ) ) {
							foreach ( $products->ids as $product_id ) {
								$GLOBALS['post'] = get_post( $product_id ); // phpcs: override ok.
								setup_postdata( $GLOBALS['post'] );
								wc_get_template_part( 'content', 'product' );
							}
						}

						$GLOBALS['post'] = $original_post; // phpcs: override ok.
...
document.addEventListener('DOMContentLoaded', function(){
	document.addEventListener('click', function(e){
		if( e.target.classList.contains('ajs-tabs') && !e.target.classList.contains('active') ){
			e.preventDefault();
			document.querySelectorAll('.ajs-tabs-content, .ajs-tabs').forEach(function(tabs){
				tabs.classList.remove('active');
			});
			e.target.classList.add('active');
			var c_id = e.target.getAttribute('data-id');
			if( c_id ) {
