...
function commercekit_ajax_save_wishlist() {
	...
	if ( $pid ) {
		...
		$ajax['status'] = 1;
		if ( 'page' === $type ) {
			$ajax['html'] = '<a href="' . $wsl_page_link . '" class="commercekit-browse-wishlist"><i class="cg-wishlist-t"></i><span>' . $wsl_brtext . '</span></a>';
		} else {
			$ajax['html'] = '<a href="' . $wsl_page_link . '" class="commercekit-browse-wishlist"><i class="cg-wishlist-t"></i><span></span></a>';
		}
...
function commercekit_ajax_remove_wishlist() {
	...
	$ajax['status'] = 1;
	$ajax['html']   = '<a href="#wishlists" class="commercekit-save-wishlist" data-product-id="' . $pid . '" data-type="list"><i class="cg-wishlist"></i><span></span></a>';
...
function commercekit_single_product_wishlist() {
	...
	if ( $commercekit_wishlist ) {
		...
		if ( $wid ) {
			...
			$wsl_page      = isset( $commercekit_options['wsl_page'] ) ? esc_attr( $commercekit_options['wsl_page'] ) : 0;
			$wsl_page_link = get_permalink( $wsl_page );
			$html          = '<div class="commercekit-wishlist"><a href="' . $wsl_page_link . '" class="commercekit-browse-wishlist"><i class="cg-wishlist"></i><span>' . $wsl_brtext . '</span></a></div>';
		} else {
			$wsl_adtext = isset( $commercekit_options['wsl_adtext'] ) && ! empty( $commercekit_options['wsl_adtext'] ) ? commercekit_get_multilingual_string( stripslashes_deep( $commercekit_options['wsl_adtext'] ) ) : esc_html__( 'Add to wishlist', 'commercegurus-commercekit' );
			$html       = '<div class="commercekit-wishlist"><a href="#" class="commercekit-save-wishlist" data-product-id="' . $pid . '" data-type="page"><i class="cg-wishlist"></i><span>' . $wsl_adtext . '</span></a></div>';
		}
...
function commercekit_after_shop_loop_item_wishlist() {
	...
	if ( $commercekit_wishlist ) {
		...
		if ( $wid ) {
			$html = '<div class="commercekit-wishlist mini"><a href="' . $wsl_page_link . '" class="commercekit-browse-wishlist"><i class="cg-wishlist-t"></i><span></span></a></div>';
		} else {
			$html = '<div class="commercekit-wishlist mini"><a href="#" class="commercekit-save-wishlist" data-product-id="' . $pid . '" data-type="list"><i class="cg-wishlist"></i><span></span></a></div>';
		}
	...
	<table class="commercekit-wishlist-table" width="100%">
		...
		<tbody>
			...
			<tr id="product-<?php echo esc_attr( $row['product_id'] ); ?>">
				<td class="remove">
					<a href="#wishlists" class="commercekit-remove-wishlist2" data-product-id="<?php echo esc_html( $row['product_id'] ); ?>" data-wpage="<?php echo esc_html( $wpage ); ?>">×</a>
				</td>
				<td class="image">
				...
				<td class="cart">
					<?php if ( $in_stock && ( $product->is_type( 'simple' ) || $product->is_type( 'variable' ) ) ) { ?>
						<?php if ( $product->is_type( 'simple' ) ) { ?>
							<button type="button" class="commercekit-wishlist-cart" data-product-id="<?php echo esc_attr( $row['product_id'] ); ?>"> <?php esc_html_e( 'Add to Cart', 'commercegurus-commercekit' ); ?> </button>
						<?php } else { ?>
